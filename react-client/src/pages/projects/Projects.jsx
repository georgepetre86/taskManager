...
export default function Projects({tab}) {
    ...
    renderCell: (params) => {
        return(
            <>  <Link to={"/projects/"+params.row._id}>
                ...
                <DeleteIcon style={{color: "#aa5042", marginLeft: "10px"}} onClick = {()=>handleDeleteProjectModal(params.row._id)}/>
  ...
  const dateFormat = (item) => {
    ...
    return ConvDate.getDate() + "/" + (parseInt(ConvDate.getMonth()) + 1) + "/" + ConvDate.getFullYear();
  }
          ...
          <div className="addNewProjectModal">
            <div className="modal-content">
              ...
              <div className="topModal">
                <h3 style={{fontWeight: "400", color: "#333"}}>Delete Project {projectNumberForDelete}</h3>
              </div>
              <div className="midModal">
                <h2>Are you sure you want to delete this project?</h2>
                <div className="deleteOptions">
                  <button className="yes" onClick={()=>handleDeleteProject()}>Yes</button>
                  <button className="no" onClick={()=>setDeleteModal(false)}>No</button>
                </div>
              </div>
